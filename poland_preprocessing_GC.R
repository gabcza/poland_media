...
dat_long <- dat_long %>% 
  left_join(dat %>% dplyr::select(
    ...
    "start_date",
    "age", "female", "income",
    # Gabriela: adding aligned vs. counter sources here
    ...
    by = "ResponseId") #%>%
# Gabriela: this bit takes a long time to execute in long data, maybe we can add it
# with individual-level data earlier? 
#mutate(counter_source = 
#         case_when(source == "tvp" & anti_pis == 1 ~ 1,
#                   source == "tvn" & pro_pis == 1 ~ 1,
#                   source == "tvp" & pro_pis == 1 ~ 0,
